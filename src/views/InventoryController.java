...
//    private TableView tbl_invent;
    ...
    public void initialize(URL url, ResourceBundle rb) {
        ...
        tbl_invent.getSelectionModel().selectedItemProperty().addListener((obs, oldSelection, newSelection) -> {
            InvenModel e = (InvenModel) obs.getValue();
...
//            System.out.println("dj");
                db.setData("INSERT INTO Inventory(Item_code,Item_name,Description,Quantity,Receive_from,Receive_date,Expire_date,Unit_price,Minqty_level) VALUES ('" + Item_code + "','" + Item_name + "','" + description + "','" + Quantity + "','" + receivefrm + "','" + receivedate + "','" + Expiredate + "','" + unit_price + "','" + min_level + "' )");
                Alert alert = new Alert(Alert.AlertType.INFORMATION);
                alert.setTitle("Information Alert");
                alert.setHeaderText("Item Adding Request");
                alert.setContentText("Item Details are added Successfully");
                alert.showAndWait();
                ObservableList<InvenModel> list = getAllInventorydata();
    ...
    public ObservableList<InvenModel> getAllInventorydata() {

        JDBC j = new JDBC();
        ObservableList<InvenModel> inventory = FXCollections.observableArrayList();
        try {
            ...
            while (rset.next()) {
                ...
                inventory.add(new InvenModel(Item_code, Item_name, Description, Quantity, receivefrm, receivedate, expiredate, unit_price, min_level));
                count++;
            }
            return inventory;
        } catch (Exception ex) {
    ...
    //redorder item list
    public ObservableList<InvenModel> getReorderInventorydata() {
    ...
    //search item
    public ObservableList<InvenModel> getSearchInventorydata(String key) {
        JDBC j = new JDBC();
        ObservableList<InvenModel> inventory = FXCollections.observableArrayList();
        try {
    ...
    public void searchData() {
        ...
        ObservableList<InvenModel> list1 = getSearchInventorydata(key);
        tbl_invent.setItems(list1);
    }

    public void loadInventoryTable() {
        TableColumn<InvenModel, String> itemcode_col = new TableColumn<>("Item Code");
        itemcode_col.setCellValueFactory(new PropertyValueFactory<>("Item_code"));

        TableColumn<InvenModel, String> itemname_col = new TableColumn<>("Item Name");
        itemname_col.setCellValueFactory(new PropertyValueFactory<>("Item_name"));

        TableColumn<InvenModel, String> description_col = new TableColumn<>("Description");
        description_col.setCellValueFactory(new PropertyValueFactory<>("description"));

        TableColumn<InvenModel, Integer> quantity_col = new TableColumn<>("Quantity");
        quantity_col.setCellValueFactory(new PropertyValueFactory<>("quantity"));

        TableColumn<InvenModel, String> receivefrm_col = new TableColumn<>("Receive From");
        receivefrm_col.setCellValueFactory(new PropertyValueFactory<>("receivefrm"));

        TableColumn<InvenModel, String> receivedate_col = new TableColumn<>("Receive Date");
        receivedate_col.setCellValueFactory(new PropertyValueFactory<>("receivedate"));

        TableColumn<InvenModel, String> expiredate_col = new TableColumn<>("Expire Date");
        expiredate_col.setCellValueFactory(new PropertyValueFactory<>("Expiredate"));

        TableColumn<InvenModel, Integer> unit_price_col = new TableColumn<>("Unit Price");
        unit_price_col.setCellValueFactory(new PropertyValueFactory<>("unit_price"));

        TableColumn<InvenModel, Integer> min_level_col = new TableColumn<>("Minimum qty Level");
        ...
        try {
            ...
            itemcode_col.setCellFactory((TableColumn<InvenModel, String> column) -> {
                return new TableCell<InvenModel, String>() {
                    @Override
                    protected void updateItem(String item, boolean empty) {
                        if (item == null || empty) {
                            ...
                            InvenModel e = (InvenModel) getTableRow().getItem();
        ...
        try {
            if (!checkInventoryEmptyFields() && checkNumericFields()) {
                ...
                if (result.get() == ButtonType.OK) {
                    db.setData("UPDATE inventory set Item_name='" + Itemname + "',Description='" + descript + "',Quantity=" + Qty + ", Receive_from='" + receivefrm + "', Receive_date='" + receivedate + "', Expire_date='" + Exprdate + "',Unit_price='" + unit_price + "', Minqty_level=" + min_level + " WHERE Item_code='" + Item_code + "'");
                    ObservableList<InvenModel> list = getAllInventorydata();
                    tbl_invent.setItems(list);
