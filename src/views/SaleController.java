...
public class SaleController implements Initializable {
    ...
    public void initialize(URL url, ResourceBundle rb) {
        ...
        lbl_customer.setVisible(false);
        cust_txt.setVisible(false);
    ...
    public ObservableList<Invoice> getsalesRecords() {
        ...
        try {
            ResultSet rset = null;
            if (!filtered) {
                rset = j.getData("SELECT * FROM sales order by date desc");
            } else {
                rset = j.getData("select * from deema_baby_fair.sales where date between '" + start + "' and '" + end + "' order by date desc");
            }
            while (rset.next()) {
    ...
    public boolean validateQty() {
        ...
        if (qty <= 0) {
            Alert alert = new Alert(Alert.AlertType.ERROR);
            alert.setTitle("Error");
            alert.setHeaderText("Quentity should be positive.");
            alert.showAndWait();
            return false;
        }
        return true;
    ...
    public boolean getStockQty() {
        ...
        try {
            ResultSet rset = j.getData("SELECT qtyOnHand FROM deema_baby_fair.stock WHERE pr_code = '" + item_code + "' ");
            ...
            if (rset.next()) {
                ...
                if (qty_on_hand < qty) {
                    ...
                    Alert alert = new Alert(Alert.AlertType.ERROR);
                    alert.setTitle("Error");
                    alert.setHeaderText("Stock quentity is not enough.");
                    alert.showAndWait();
                    return false;
                }
            }
    ...
    public boolean ValidatereturnQty() {
        if (ret_qty_txt.getText().isEmpty()) {
            Alert alert = new Alert(Alert.AlertType.ERROR);
            alert.setTitle("Error");
            alert.setHeaderText("Return Quentity cannot Empty");
            alert.showAndWait();
            return false;
        }
        ...
        if (ret_qty > qty || ret_qty == 0) {
            Alert alert = new Alert(Alert.AlertType.ERROR);
            alert.setTitle("Error");
            alert.setHeaderText("Return Quentity is Invalid.");
            alert.showAndWait();
            return false;
        } else if (ret_qty < 0) {
            Alert alert = new Alert(Alert.AlertType.ERROR);
            alert.setTitle("Error");
            alert.setHeaderText("Return qty cannot be negative.");
            alert.showAndWait();
            return false;
        } else {
            return true;
    ...
    /**
     * send invoice details to database.
     */
    ...
    public void searchItem() {
        ...
        String item_code = item_txt.getText();
        try {
            ResultSet rset = j.getData("SELECT pr_code,description,unitPrice FROM stock WHERE pr_code = '" + item_code + "' ");
    ...
    /**
     * load exchange item description
     */
 ...
 */
    ...
    /**
     * delete row from exchange table    
    */
